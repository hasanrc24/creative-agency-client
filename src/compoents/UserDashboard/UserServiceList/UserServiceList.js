...
const UserServiceList = () => {
    ...
    useEffect(() =>{
            fetch(`https://safe-inlet-61017.herokuapp.com/myOrders?email=${loggedInUser.email}`)
            .then(res=>res.json())
            .then(data=> {
                setMyService(data)
                setLoading(false)
        ...
        <div>
            ...
            <div>
                <Row className="user-service">
                    ...
                    {
                        myService.length ? myService.map((myService, index) => <UserServiceCard myService={myService} key={index} />) :
