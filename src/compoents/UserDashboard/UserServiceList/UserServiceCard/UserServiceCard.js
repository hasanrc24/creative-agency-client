...
const UserServiceCard = ({myService}) => {
    ...
    const myServices = allService.filter(srvs => srvs.service.serviceTitle === orderTitle);
    return (
        <Col md={4}>
            {
                allService.length ? 
                <Card className="mb-3 user-service-card">
                    <Card.Body className="d-flex justify-content-between align-items-center">
                        <Image style={{height:"50px"}} src={`data:image/png;base64,${myServices[0].icon}`} />
                        <span className={
                    ...
                    <Card.Body>
                        ...
                        <p className="text-secondary">{myServices[0].service.serviceDesc}</p>
                    </Card.Body>
                </Card>
                : 
                <CircularProgress />
